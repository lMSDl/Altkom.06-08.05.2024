...
namespace Warehouse
{
    internal abstract class GenericAsyncProgram<T> where T : Entity
    {

        IEntityAsyncService<T> _service;

        public GenericAsyncProgram(IEntityAsyncService<T> service)
        {
            _service = service;
        }
        ...
        {
            int id = GetInt("Id");
            T item = await _service.ReadAsync(id);
            if (item == null)
                return null;
        ...
        {
            int id = GetInt("Id");
            T old = await _service.ReadAsync(id);
            if (old == null)
                return;
        ...
        async void Show()
        {
            foreach (T p in await _service.ReadAsync())
            {
